export default function TopBar({ roomId, roomName, onlineCount = 0 }) {
  console.log('TopBar props:', { roomId, roomName })
  const CHANNEL_IDS = ['general', 'random', 'dev-talk']
  const isDM = !CHANNEL_IDS.includes(roomId)

  // Always use roomName prop, but fallback to roomId only for channels
  ...
  return (
    <div className="flex items-center gap-3 px-5 py-3.5 border-b border-zinc-800 flex-shrink-0">
      <div className="flex items-center gap-1.5 flex-1 min-w-0">
        ...
        <span className="text-white text-sm font-semibold truncate capitalize">
          {displayName}
        </span>
        {!isDM && (
...
function IconBtn({ label, children }) {
  return (
    <button className="w-8 h-8 rounded-lg hover:bg-zinc-800 flex items-center justify-center transition-colors" aria-label={label} type="button">
      {children}
    </button>
  )
}