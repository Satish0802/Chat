...
export default function TopBar({ roomId, roomName, onlineCount = 0, messages = [], onRoomSelect, unreadCounts = {} }) {
  const CHANNEL_IDS = ['general', 'random', 'dev-talk']
  const isDM = !CHANNEL_IDS.includes(roomId)
    ...
    <div className="relative flex-shrink-0">
      ...
      <div className="flex items-center gap-3 px-5 py-3.5 border-b border-zinc-800">
        ...
        <div className="flex items-center gap-1 text-zinc-500">
          {/* Search */}
          <IconBtn label="Search messages" onClick={() => { setSearchOpen(o => !o); setSearchQuery(''); setPanelOpen(false); setNotifOpen(false) }} active={searchOpen}>
          ...
          {/* Members / Info panel */}
          <IconBtn label={isDM ? 'Shared media' : 'Members & media'} onClick={() => { setPanelOpen(o => !o); setSearchOpen(false); setNotifOpen(false) }} active={panelOpen}>
            <svg xmlns="http://www.w3.org/2000/svg" className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
          ...
          <div className="relative">
            <IconBtn label="Notifications" onClick={() => { setNotifOpen(o => !o); setSearchOpen(false); setPanelOpen(false) }} active={notifOpen}>
              <svg xmlns="http://www.w3.org/2000/svg" className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
          ...
          {unreadRooms.length === 0 ? (
            ...
            <div className="max-h-64 overflow-y-auto">
              {unreadRooms.map(([room, count]) => (
                <button
                  key={room}
                  onClick={() => { onRoomSelect && onRoomSelect(room); setNotifOpen(false) }}
      ...
      {/* Info/Media panel — slides in from right as overlay */}
      {panelOpen && (
        <div className="absolute top-full right-0 z-50 w-72 bg-zinc-900 border border-zinc-800 rounded-xl shadow-xl mt-1 mr-4 overflow-hidden max-h-[80vh] flex flex-col">
          <p className="text-xs font-semibold text-zinc-400 px-4 py-3 border-b border-zinc-800 flex-shrink-0">
            {isDM ? 'Shared media & files' : 'Members & media'}
          </p>
          ...
          <div className="overflow-y-auto flex-1">
            ...
            {sharedImages.length > 0 && (
              <div className="px-4 py-3 border-b border-zinc-800">
                <p className="text-[10px] text-zinc-500 uppercase tracking-widest mb-2">Images</p>
            ...
            {sharedFiles.length > 0 && (
              <div className="px-4 py-3 border-b border-zinc-800">
                <p className="text-[10px] text-zinc-500 uppercase tracking-widest mb-2">Files</p>
            ...
            {sharedImages.length === 0 && sharedFiles.length === 0 && (
              <p className="text-zinc-500 text-xs text-center py-6">No shared media yet</p>
            )}
          </div>
        </div>
      )}
